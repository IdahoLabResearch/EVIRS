...
class tabbedRequestHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if idmode == 'vinbtn':
            ...
            submodel = self.get_argument("submodellist")
            vin = ""
            vinInfo = GetVehicleInfo(make, model, year, submodel)
        ...
        self.render("indextabbed.html", 
            ...
            kWh=kWh, 
            imgfigure=imgfigure, 
            imgstructure=imgstructure, 
            imgtransport=imgtransport)

class getSubmodelsByVINRequestHandler(tornado.web.RequestHandler):
    def post(self, *args):
...
class getMakesRequestHandler(tornado.web.RequestHandler):
    def get(self, *args):
...
class getModelsRequestHandler(tornado.web.RequestHandler):
    def post(self, *args):
...
class getYearsRequestHandler(tornado.web.RequestHandler):
    def post(self, *args):
...
class getSubmodelsRequestHandler(tornado.web.RequestHandler):
    def post(self, *args):
        model = tornado.escape.json_decode(self.request.body)['model']
        make = tornado.escape.json_decode(self.request.body)['make']
